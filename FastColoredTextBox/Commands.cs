...
namespace FastColoredTextBoxNS
{
    ...
    internal class InsertCharCommand: UndoableCommand
    {
        ...
        public override void Undo()
        {
            ...
            switch (c)
            {
                ...
                default:
                    ...
                    tb.lines[sel.Start.iLine].RemoveAt(sel.Start.iChar);
        ...
        {
            if (tb.lines.Count == 0)
                tb.lines.Insert(tb.Selection.Start.iLine + 1, new Line());
            else
    ...
    internal class ClearSelectedCommand : UndoableCommand
    {
        ...
        internal static void ClearSelected(FastColoredTextBox tb)
        {
            ...
            //
            if (fromLine == toLine)
                tb.lines[fromLine].RemoveRange(fromChar, toChar - fromChar);
            else
            {
                tb.lines[fromLine].RemoveRange(fromChar, tb.lines[fromLine].Count - fromChar);
                tb.lines[toLine].RemoveRange(0, toChar);
                tb.lines.RemoveRange(fromLine + 1, toLine - fromLine - 1);
